...
class TrafficWeak(QMainWindow):
    def __init__(self): # 생성자
        ...
        self.signFiles = [["C:\ComputerVision\source\ch6\child.png",'어린이'],
                          ["C:\ComputerVision\source\ch6\disabled.png",'장애인'],
                          ["C:\ComputerVision\source\ch6\elder.png",'노인']]
        self.signImgs=[]

    def signFunction(self): # 표지판 영상을 읽어 signImgs에 저장하고 윈도우에 디스플레이
        self.label.clear()
    ...
    def recognitionFunction(self):
        if self.roadImg is None:
            ...
            KD = []  # 여러 표지판 영상의 키포인트와 기술자 저장, KD = [[특징점1, 기술자1], [특징점2, 기술자2],[특징점3, 기술자3]]
            ...
            road_kp, road_des = sift.detectAndCompute(grayRoad, None)  # 도로영상 키포인트와 기술자 추출

            matcher = cv.DescriptorMatcher_create(cv.DescriptorMatcher_FLANNBASED) # FLANN 기반 매칭
            GM = []  # 여러 표지판 영상의 good match를 저장 (매칭 결과 저장)
            for sign_kp, sign_des in KD:
                knn_match = matcher.knnMatch(sign_des, road_des, 2) # 특징점마다 최근접 이웃 2개 찾음
                T = 0.7
                good_match = []
            ...
            if len(GM[best]) < 4:  # 최선의 번호판이 매칭 쌍 4개 미만이면 실패
                ...
                sign_kp = KD[best][0] # KD의 best 요소
                good_match = GM[best] # GM의 best 요소
